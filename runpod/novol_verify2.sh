...
( while :; do sleep 1800
    [ -s "$OUT/best.pt" ] && $PY runpod/s3io.py up "$OUT/best.pt" final/verifier2_partial.pt >/dev/null 2>&1
  done ) &
KEEPER=$!
...
if $PY verify2.py train --shard "$SHARD" --ckpt "$TRUNK" --out "$OUT" \
        --k 5 --players 40000 --min-games 6 \
        --neighbours 64 --mlpg 60 \
        --batch 48 --batches-per-epoch 600 --eval-batches 40 \
        --eval-every 800 --patience 12 \
        --workers 16 --lr 6e-5 --max-hours "$FT_H"; then
    echo VERIFY2_TRAIN_DONE
else
    echo VERIFY2_TRAIN_FAILED
fi
kill "$KEEPER" 2>/dev/null

for f in best.pt last.pt history.json; do
    [ -s "$OUT/$f" ] && $PY runpod/s3io.py up "$OUT/$f" "final/verifier2_$f"
done

echo "=== $(date -u +%H:%M:%S) uploading ==="
cp -f "$LOG" "$D/verify2_run.log" 2>/dev/null
$PY runpod/s3io.py up "$D/verify2_run.log" final/verify2_run.log || true
